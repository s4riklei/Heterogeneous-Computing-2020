...
namespace {
	...
	__global__ void calculateBestRoute(const int threadCount, const int prefixSize, const int pointCount, const DevicePointers d) {
		...
		float bestResult = calculatePermutationTotalDistance(d.distanceMatrix, permutation, pointCount);
		...
		if (permutableSize != 0) {
			...
			while (i < permutableSize) {
				if (c[blockDim.x * i] < i) {
					...
					float tempResult = calculatePermutationTotalDistance(d.distanceMatrix, permutation, pointCount);
	...
	void executeThreads(const Metadata& metadata, const DevicePointers& d) {
		...
		calculateBestRoute<<<blockCount, metadata.threadCountPerBlock>>>(metadata.threadCount, metadata.prefixSize, metadata.pointCount, d);
	}
